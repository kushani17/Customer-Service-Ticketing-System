...
const KEYS_TO_FILTERS = ['name','company','phone','email','city','state','refferedby'];
  ...
  componentDidMount = () => {
    ...
    getpostdata =() => {
        ...
        .catch(() =>{
         ...
         renderProducts() {
             return filteredContent.map(product => {
              return (
                <tr style={{color:"White"}} key={product._id} onClick={() => {this.props.history.push({pathname:"/updateclient/",state:{ id: product._id}})}}>           
                  <td>{product.name}</td>
                  ...
                  <td>{product.city}</td>
                  <td>{product.state}</td>
                  <td>{product.refferedby}</td>
                  <td><div 
        onClick={event => event.stopPropagation()}>
           <DeleteIcon onClick={() => {this.deleteTicket(product._id)}}/></div>
                  </td>
                </tr>
              );
            })
        ...
        render()
      {
        ...
        return(
          <div>
        <h2 style={{color:'#f2f3f5', fontFamily:'Times New Roman',fontSize:32, margin: 5, textAlign:'center', marginTop:30 }}><b>Client Information</b></h2>
        <div>
      <SearchInput className="search-input" style={{width:220, marginLeft:"auto", marginBottom: 10}} placeholder="  Search"  onChange={this.searchUpdated}/>
          ...
          <table className="table table-hover">    
            <thead className="thead-light">
              ...
              <th style={{textAlign:'center', borderColor:'black', borderStyle:'solid'}}>Company</th>
              ...
              <th style={{textAlign:'center', borderColor:'black', borderStyle:'solid'}}>RefferedBy</th>
